...
public class OmitSegmentsUrlProvider : DefaultUrlProvider
{
    ...
    private string ConstructUrl(UmbracoContext umbracoContext, int id, Uri current, UrlProviderMode mode, IPublishedContent content, OmitSegmentsRule rule)
    {
        ...
        foreach (string p in urlParts)
        {
            ...
            if (rule.ExcludeDocTypeAliases.Split(',').Any(x => x.Equals(docTypeAlias)))
            {
                urlParts[cnt] = "";
            }
            cnt++;
        ...
        //will be our final url, without the parts that correspond to excluded doctypes.
